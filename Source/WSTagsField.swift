...
open class WSTagsField: UIScrollView {
    ...
    open var shouldScrollHorizontally : Bool = false
    ...
    open func scrollViewToLastTag() {
        if (self.shouldScrollHorizontally == true) {
            self.repositionViewsForHorizontalScrolling(shouldScrollToLast: true)
        }
    }
...
extension WSTagsField {
    ...
    fileprivate func enumerateItemRects(layoutWidth: CGFloat, using closure: (_ tagView: WSTagView?, _ tagRect: CGRect?, _ textFieldRect: CGRect?) -> Void) {
        ...
        var availableWidthForTextField: CGFloat
        if (self.shouldScrollHorizontally == false) {
            availableWidthForTextField = maxWidth - curX
        } else {
            let mw : CGFloat = self.bounds.width - contentInset.left - contentInset.right
            availableWidthForTextField = mw - curX
        }
        ...
        if textField.isEnabled {
            ...
            if availableWidthForTextField < Constants.MINIMUM_TEXTFIELD_WIDTH {
                ...
                if (self.shouldScrollHorizontally == false) {
                    ...
                    availableWidthForTextField = 160
                }
    ...
    fileprivate func repositionViewsForHorizontalScrolling (shouldScrollToLast : Bool = false) {
        var contentRect: CGRect = .null
        enumerateItemRects(layoutWidth: CGFloat.greatestFiniteMagnitude) { (tagView: WSTagView?, tagRect: CGRect?, textFieldRect: CGRect?) in
    ...
    fileprivate func repositionViews() {
        if self.shouldScrollHorizontally == true {
            self.repositionViewsForHorizontalScrolling()
            return
        }
